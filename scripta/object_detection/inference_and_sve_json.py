...
def save_result_to_json(result, filename=None):
    ...
    if filename is None:
        ...
        filename = f"../data_output/inference_result/hybrid_inference_result_{timestamp}.json"
...
def main():
    ...
    if result:
        ...
        print("3. 保存结果到JSON文件...")
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"../data_output/inference_result/complete_directory_test_{timestamp}.json"
        os.makedirs("../../data_output/inference_result", exist_ok=True)
